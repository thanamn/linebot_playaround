...
def run_broadcast():
    try:
        # 1. Get current time and the prompt variable
        tz = pytz.timezone('Asia/Bangkok')
        now = datetime.now(tz)
        
        # Get prompt from GitHub Variable
        raw_prompt = os.getenv('BOT_PROMPT', "Say hello in Japanese.")
        
        # Fill in the {current_date} and {current_time} placeholders
        final_prompt = raw_prompt.format(
            current_date=now.strftime("%A, %d %B %Y"),
            current_time=now.strftime("%I:%M %p")
        )

        # 2. Generate Content with Gemini 2.5 Flash
